...
Алгоритм массового поиска имеет высокую вычислительную сложность (O(h × сумма длин подстрок)). Распараллеливание на GPU позволяет обрабатывать тысячи ячеек матрицы R одновременно, что даёт значительный выигрыш во времени на больших объёмах данных. 
...
Как видно из графиков, время CPU и GPU увеличивается практически квадратично с увеличением объема данных. Однако при использовании GPU тратится меньшее количество времени, начиная с размера = 320. 

Из результатов эксперимента видно, что параллелизация необходима для больших объёмов данных, но текущая реализация имеет ограничения из-за активного использования атомарных операций и неоптимального отображения задачи на GPU. Совпадение результатов CPU и GPU говорит о корректности алгоритма.
